...
def real_traffic_data(filepath, telemetry_x, unit, experiment_duration):
    x, y = [0], [0]
    grouped_amt_bytes = 0
    current_time = 0
    previous_time = 0
        ...
        for line in txt_file:
            ...
            if hop_cnt == 0:
                ...
                if(cols[0]==switch_id):
                    ...
                    # time_window_s = float("{:.2f}".format(time_window_s))
...
def save_rmse_and_telemetry_byte_count(args, sw_type, real_y_expanded, telemetry_y_expanded, area_between, telemetry_byte_count, telemetry_pkts_count):
    rmse = math.sqrt(np.square(np.subtract(real_y_expanded, telemetry_y_expanded)).mean())

    print(rmse, rmse/(max(real_y_expanded) - min(real_y_expanded)), rmse/(sum(real_y_expanded)/len(real_y_expanded)), area_between)
    rmse = rmse/(max(real_y_expanded) - min(real_y_expanded))
    
    filepath = args['rmse_output_folder']+args['traffic_shape']+".csv"
    file_exists = os.path.isfile(filepath)
    y_set = sorted(set(real_y_expanded))
    with open(filepath, "a") as csvfile:
        headers = ['sw_type', 'sw_id', 'min_telemetry_push_time', 'experiment_time', 'packet_count', 'rmse', 'area_between', 'telemetry_byte_count']
        ...
        writer.writerow({'sw_type': sw_type, 'sw_id':args['switch_id'], 'min_telemetry_push_time': args['min_telemetry_push_time'], 
                    'experiment_time': args['experiment_duration'], 'packet_count': telemetry_pkts_count, 
                        'rmse': rmse, 'area_between': area_between, 'telemetry_byte_count': telemetry_byte_count})
...
def plot_line_graph(args, sw_type, real_data, telemetry_data):
    ...
    while j<len(real_x) and i<len(x_combined):
        if round(x_combined[i]) <= real_x[j]:
    ...
    for idx, value in enumerate(telemetry_y):
        print(telemetry_x[idx], value)

    for idx, value in enumerate(x_combined):
        print(value, real_y_expanded[idx], telemetry_y_expanded[idx])
    ...
    return temp_real_y_expanded, temp_telemetry_y_expanded, math.fabs(np.trapz(y=np.subtract(real_y_expanded, telemetry_y_expanded), x=x_combined))
...
def main():
    ...
    for f in telemetry_files:
        ...
        real_x, real_y = real_traffic_data(real_traffic_file, telemetry_x, args['unit'], args['experiment_duration'])
        ...
        real_y_expanded, telemetry_y_expanded, area_between = plot_line_graph(args, sw_type, (real_x, real_y), telemetry_data.get(sw_type))
        save_rmse_and_telemetry_byte_count(args, sw_type, real_y_expanded, telemetry_y_expanded, area_between, telemetry_byte_count, len(telemetry_x))
